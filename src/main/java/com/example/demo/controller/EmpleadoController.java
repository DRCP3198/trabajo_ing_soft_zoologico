...
public class EmpleadoController {
	...
	@GetMapping("/HistorialAnimal/{id}")
	public String listaCitaAnimal(@PathVariable("id") Integer id,Model modelo) {
		List<HistorialClinico> historiales = this.clinicoService.buscarTodos();
		HistorialClinico historial=new HistorialClinico();
		Animal a=this.animalService.buscarPorId(id);
		
		for(HistorialClinico h:historiales) {
			List<citaHistorialClinico> citasH=h.getCitas();
			for(citaHistorialClinico c:citasH) {
				if(c.getAnimal().getId().equals(a.getId())){
					historial=h;
				}
			}
		}
		modelo.addAttribute("historial", historial);
		return "vistaHistorialDeAnimal";
	}
	
	@GetMapping("/listaCitasAnimal/{id}")
	public String listaCitasHistorialAnimal(@PathVariable("id") Integer id,Model modelo) {
		List<citaHistorialClinico> citas = this.citasHistorialclinicoService.buscarTodos();
		List<citaHistorialClinico> citasAnimal=new ArrayList<>();
		for(citaHistorialClinico c:citas) {
			if(c.getHistorial_clinico().getId().equals(id)) {
				citasAnimal.add(c);
			}
		}
		
		
		modelo.addAttribute("citasAnimal", citasAnimal);
		return "vistaListaCitasAnimal";
	}
	
	//HISTORIAL CLINICO
	@GetMapping("/registrarHistorial/{id}")
	public String registrarHistorial(@PathVariable("id") Integer id,Model model) {
		Animal a=this.animalService.buscarPorId(id);
		List<citaHistorialClinico>citas= a.getCitas();
